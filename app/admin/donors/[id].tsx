...
export default function AdminDonorDetails() {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      const updated = await AdminService.updateDonor(donor._id, {
        name: form.name,
        email: form.email,
  ...
  const toggleStatus = async () => {
    ...
    try {
      const updated = await AdminService.updateDonorStatus(donor._id, newStatus, newStatus === 'suspended' ? 'Suspended by admin' : undefined);
      setDonor(updated);
    } catch (e) {
  ...
  const toggleVerify = async () => {
    if (!donor) return;
    try {
      const updated = await AdminService.verifyDonor(donor._id, !donor.verified, donor.verified ? undefined : 'Verified by admin');
      setDonor(updated);
    } catch (e) {
  ...
  const remove = async () => {
    ...
    Alert.alert('Delete Donor', 'Are you sure you want to permanently delete this donor?', [
      ...
      { text: 'Delete', style: 'destructive', onPress: async () => {
        try {
          await AdminService.deleteDonor(donor._id);
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        <View style={styles.section}>
          ...
          <View style={styles.row}><Droplet size={18} color="#6B7280" /><TextInput style={styles.input} value={form.bloodGroup || ''} onChangeText={(t)=>setForm((f)=>({...f,bloodGroup:t}))} placeholder="Blood Group" placeholderTextColor="#9CA3AF" /></View>
          ...
          <View style={styles.row}><Phone size={18} color="#6B7280" /><TextInput style={styles.input} value={form.phoneNumber || ''} onChangeText={(t)=>setForm((f)=>({...f,phoneNumber:t}))} placeholder="Phone Number" placeholderTextColor="#9CA3AF" keyboardType="phone-pad" /></View>
        </View>
        ...
        <View style={styles.section}>
          ...
          {donor.verificationNote ? <Text style={styles.note}>Note: {donor.verificationNote}</Text> : null}
        </View>
